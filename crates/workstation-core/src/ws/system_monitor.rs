//! System monitor bridge (T18.5 desktop, T19.28 cloud-agent).
//!
//! A single background tokio task polls [`sysinfo`] on a fixed cadence
//! (default: 4 seconds) and broadcasts the resulting snapshot to every
//! subscribed WebSocket connection. Per-connection forwarders living
//! inside each backend's dispatch loop pull from the broadcast channel
//! and ship `system_stats` frames through the same outbound mpsc that
//! carries PTY frames.
//!
...
//!
//! This module is Tauri-free and lives in `workstation-core` so the
//! desktop bridge (`src-tauri/src/ws`) and the headless cloud-agent
//! (`crates/cloud-agent`) share a single implementation.
...
/// Start the monitor on a tokio task with the default interval
/// ([`DEFAULT_INTERVAL`]). The returned handle is cheap to clone and
/// can be passed across the application; the underlying broadcast
/// sender keeps the publisher task alive even when no receivers exist.
pub fn start(manager: PtyManager) -> SystemMonitorHandle {
    start_with_interval(manager, DEFAULT_INTERVAL)
}

/// Same as [`start`] but with a caller-supplied tick period. Test
/// callers use a sub-second period to keep `cargo test` snappy.
pub fn start_with_interval(manager: PtyManager, period: Duration) -> SystemMonitorHandle {
