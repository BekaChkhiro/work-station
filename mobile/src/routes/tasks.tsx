  ...
  return (
    <div class="mt-4 flex flex-col gap-3">
      <Show when={!props.loading && props.error}>
        <ErrorBanner error={props.error} onRetry={props.onRetry} onClearToken={props.onClearToken} />
      </Show>
...
function TaskRow(props: { task: Task; onOpen: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span
        class={`h-2 w-2 shrink-0 rounded-full ${complexityClass()}`}
        aria-hidden="true"
      />
      ...
      <Show when={(props.task.dependencies?.length ?? 0) > 0}>
        <span class="text-fg-tertiary text-[10px]">↳{props.task.dependencies!.length}</span>
      </Show>
    </button>
  );
}
        ...
        >
          ...
          >
            ...
            <Show when={task().description}>
              <p class="text-fg-secondary mt-3 whitespace-pre-wrap text-sm">
                {task().description}
              </p>
            </Show>
