...
describe("PlanFlowClient.getUnreadNotificationCount", () => {
  ...
  it("rejects when the payload is malformed (count missing)", async () => {
    const fetchImpl = makeFetch(() => jsonResponse({ unread: 3 }));
...
describe("PlanFlowClient.listNotifications", () => {
  it("returns the list as-is, preserving optional fields", async () => {
    const fetchImpl = makeFetch(() =>
      jsonResponse([
        {
          id: "n-1",
          kind: "mention",
          message: "Alice mentioned you on T12.9",
          read: false,
          createdAt: "2026-05-11T12:00:00.000Z",
          projectId: "pf-1",
          taskId: "T12.9",
        },
        { id: "n-2", kind: "system", message: "Welcome" },
      ]),
    );
    ...
    expect(list[1]?.read).toBeUndefined();
  });
});

describe("PlanFlowClient.markNotificationRead", () => {
  it("POSTs to /notifications/:id/read and resolves to void", async () => {
    ...
    expect(calls).toEqual([
      { url: "https://api.planflow.tools/notifications/n-123/read", method: "POST" },
    ]);
  });
